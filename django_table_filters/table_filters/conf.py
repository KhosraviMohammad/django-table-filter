...
def generate_column_filter(*, column, table, model, split_column_names, name_and_fields, fields_and_models):
    ...
    if last_field.choices is not None:
        column_filter = ColumnFilter({filter_accessor: filters.ChoiceFilter(choices=last_field.choices, widget=forms.TextInput(attrs={'class': 'form-control'}))})
    elif isinstance(last_field, models.ManyToManyField):
        last_model = fields_and_models[last_field]
        column_filter = ColumnFilter({filter_accessor: filters.ModelMultipleChoiceFilter(queryset=last_model.objects.all(), widget=forms.SelectMultiple(attrs={'class': 'form-control'}))})
        ...
        filter_from = filters.NumberFilter(field_name=filter_accessor, lookup_expr='gte', widget=forms.NumberInput(attrs={'class': 'form-control'}))
